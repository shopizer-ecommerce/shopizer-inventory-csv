...
public class ProductTypeImport {
	...
	public void process() throws Exception {
		...
		for (CSVRecord record : parser) {
			...
			/*
			 * 
			 * if(StringUtils.isBlank(record.get("type_en"))) { continue; }
			 * 
			 *//**
				 * temp code generate friendly code
				 *//*
					 * //System.out.println(this.codeGen(record.get("type_en"))); //continue;
					 * 
					 * PersistableProductType type = new PersistableProductType();
					 * type.setCode(record.get("code")); type.setAllowAddToCart(true);
					 * 
					 * type.setId(null);
					 * 
					 * 
					 * List<ProductTypeDescription> descriptions = new
					 * ArrayList<ProductTypeDescription>();
					 * 
					 * //add english description ProductTypeDescription description = new
					 * ProductTypeDescription(); description.setLanguage("en");
					 * description.setName(record.get("type_en"));
					 * description.setDescription(record.get("type_en")); description.setId(null);
					 * 
					 * 
					 * descriptions.add(description);
					 * 
					 * //add other description description = new ProductTypeDescription();
					 * if(headerMap.containsKey("type_fr")) { description.setId(null);
					 * description.setLanguage("fr"); description.setName(record.get("type_fr"));
					 * description.setDescription(record.get("type_fr")); } else {
					 * description.setId(null); description.setLanguage("fr");
					 * description.setName(record.get("type_en"));
					 ...
					 * 
					 * @SuppressWarnings("rawtypes") ResponseEntity response =
					 * restTemplate.postForEntity(endPoint + MERCHANT, entity,
					 * PersistableProductType.class);
					 * 
					 * i++; System.out.println("--------------------- [" +
					 * response.getStatusCodeValue() + "]" + i);
					 */
